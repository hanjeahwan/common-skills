...
description: Start or continue parallel coding work as the GPT-6 Astra coordinator at medium reasoning effort that performs mandatory code review while GPT-5.6 Luna workers run at max reasoning effort. Use when a coding task has multiple independent investigation or implementation units; when the user asks for a Luna swarm, Codex-Luna swarm, parallel Luna workers, or Astra-reviewed Luna workers; or when related work continues, repairs, validates, or reviews an active Swarm. When invoked, require the Astra coordinator to evaluate a useful Swarm split before proceeding without fan-out.
---

# Codex–Luna Swarm

Use one execution pattern: Luna workers investigate or implement in parallel, then the Astra coordinator personally reviews the combined code before delivery. Parallelism provides speed; the Astra coordinator review gate provides safety. Do not split these into separate modes.

```mermaid
flowchart TD
    S[Astra Coordinator Splits Tasks] --> L1[Luna Worker 1]
    S --> L2[Luna Worker 2]
    S --> LN[Luna Worker N]
    L1 --> G[Collect Changes And Evidence]
    L2 --> G
    LN --> G
    G --> R[Astra Coordinator Performs Code Review]
...
Use `Astra coordinator` as the canonical name for the coordinating and reviewing actor. Use `Luna worker` as the canonical name for each delegated actor. Keep `agent tree` and `subagent` only for their runtime and tool meanings.

| Work | Astra coordinator | Luna workers |
|---|---|---|
...
| Code review | Perform it personally | Never substitute for the Astra coordinator's review |
| Conflict resolution and delivery | Retain final decision authority | Do not make the final decision |

## Invariants

- Host-role binding: The host agent executing this Skill is the Astra coordinator for the entire Swarm, using `model: "gpt-6-astra"` and `reasoning_effort: "medium"`.
...
- Every code change must pass the Astra coordinator's direct review, including code the Astra coordinator wrote or repaired itself. A Luna worker's summary, test result, or self-assessment cannot replace this gate.
...
- When this Skill is invoked, the Astra coordinator must evaluate the task against the Swarm condition in Step 1 before proceeding alone.
- Never invent work merely to increase the Luna worker count.
- The Astra coordinator retains every approval decision.
- The Astra coordinator applies the current authorization and safety boundaries to delegated work.
...
- Direct-execution action: The Astra coordinator proceeds alone and states the concrete constraint.
...
- Direct-execution action: The Astra coordinator completes the remaining work and applies the integration, review, and validation gates.
...
- Evidence collection: The Astra coordinator reads every Luna worker result.
- Workspace inspection: The Astra coordinator inspects the actual workspace.
...
### 5. Astra Coordinator Code Review Gate

The Astra coordinator personally reviews the combined result before declaring success:
...
- Self-authored condition: The Astra coordinator implemented or repaired part of the change itself.
...
- Review-failure action: The Astra coordinator identifies a concrete defect and acceptance condition.
- Untraceable-change action: The Astra coordinator rejects or reverts the change instead of carrying it into delivery.
...
- Astra coordinator repair condition: The repair does not need Luna worker context and remains within the Astra coordinator's existing authority.
- Astra coordinator repair action: The Astra coordinator fixes the defect directly.
...
- Re-entry gate: Every repair returns to Step 4 and passes the complete Astra coordinator review and validation gate before delivery.
...
- what the Astra coordinator found during its own code review;
...
Only the Astra coordinator can mark the overall task complete.
...
- The Astra coordinator independently inspects the integrated code and verification evidence before delivery.
