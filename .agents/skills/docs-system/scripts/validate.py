...
def check_proposals(root: str, errors: list[str]) -> None:
    ...
    for rel in files:
        ...
        unexpected = [h for h in headings if h not in PROPOSAL_SECTIONS]
        if unexpected:
            errors.append(f"{rel}: unexpected H2 section(s): {unexpected}")
        core_present = [h for h in headings if h in core]
        if core_present != [s for s in core if s in headings]:
            errors.append(f"{rel}: proposal sections out of order")
        if "Outcome" in headings:
            if status not in TERMINAL_STATUSES:
                errors.append(
                    f"{rel}: `## Outcome` is only allowed for implemented/"
                    f"rejected/superseded proposals")
            elif headings[-1] != "Outcome":
                errors.append(f"{rel}: `## Outcome` must be the final section")
        elif status in TERMINAL_STATUSES:
            errors.append(
                f"{rel}: terminal proposal (status: {status}) requires "
                f"final `## Outcome`")
        for section in PROPOSAL_SECTIONS:
            if section in headings and not _section_body(text, section).strip():
    ...
    for rel in files:
        ...
        if body_id is None:
            ...
            errors.append(
                ...
                f"D{int(numeric)}")
        headings = H2_RE.findall(text)
        unknown = [h for h in headings if h not in DECISION_ORDER]
        if unknown:
            errors.append(f"{rel}: unexpected H2 section(s): {unknown}")
        ordered = [h for h in DECISION_ORDER if h in headings]
        if ordered != [h for h in headings if h in DECISION_ORDER]:
            errors.append(f"{rel}: decision sections out of template order")
        for section in DECISION_REQUIRED:
            if section not in headings:
                errors.append(f"{rel}: missing required section `## {section}`")
            elif not _section_body(text, section).strip():
                errors.append(f"{rel}: section `## {section}` is empty")
...
def check_templates(root: str, errors: list[str]) -> None:
    ...
    for name, sections in expected.items():
        ...
        if not os.path.isfile(path):
            ...
            continue
        headings = H2_RE.findall(read(path))
        if headings != sections:
            errors.append(
                f"templates/{name}: expected sections {sections}, "
                f"got {headings}")
    ...
    if os.path.isfile(index_path):
        ...
        if headers != INDEX_HEADERS:
            errors.append(
                f"templates/index.md: expected table headers {INDEX_HEADERS}, "
                f"got {headers}")
